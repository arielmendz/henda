...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/henda-release.XXXXXX")
...
app_dir="$work_dir/Henda.app"
mkdir -p "$app_dir/Contents/MacOS" "$app_dir/Contents/Resources"
lipo -create "$arm_bin/Henda" "$intel_bin/Henda" -output "$app_dir/Contents/MacOS/Henda"
cp "$root_dir/support/Info.plist" "$app_dir/Contents/Info.plist"
cp "$root_dir/support/Henda.icns" "$app_dir/Contents/Resources/Henda.icns"
cp "$root_dir/LICENSE" "$app_dir/Contents/Resources/LICENSE"
codesign --force --deep --sign - "$app_dir"

dist_dir="$root_dir/dist"
archive="$dist_dir/Henda-$version.zip"
mkdir -p "$dist_dir"
ditto -c -k --sequesterRsrc --keepParent "$app_dir" "$archive"

codesign --verify --deep --strict "$app_dir"
lipo -archs "$app_dir/Contents/MacOS/Henda"
